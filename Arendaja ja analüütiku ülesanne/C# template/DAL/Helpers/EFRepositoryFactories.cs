...
using DAL.Repositories.Domain_objects;
    ...
    public class EFRepositoryFactories : IDisposable
    {
        ...
        private static IDictionary<Type, Func<IDbContext, object>> GetCustomFactories()
        {
            return new Dictionary<Type, Func<IDbContext, object>>
                {
					{typeof (IDiseaseRepository), dbContext => new DiseaseRepository(dbContext) },
					{typeof (ISymptomRepository), dbContext => new SymptomRepository(dbContext) }
				};
        }
